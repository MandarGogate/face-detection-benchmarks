# https://github.com/ipazc/mtcnn
# pip install mtcnn

import statistics
import time
from pathlib import Path

import cv2
from mtcnn import MTCNN
import imutils


def benchmark_mtcnn_tensorflow(width=1280, height=720, rep=1, save_out=False):
    detector = MTCNN()
    time_taken = []
    for _ in range(rep):
        for img_path in Path("./test_data").iterdir():
            image = cv2.imread(str(img_path))
            image = imutils.resize(image, width=width, height=height)
            rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            tic = time.time()
            detections = detector.detect_faces(rgb_image)
            time_taken.append(time.time() - tic)
            if detections is None:
                continue
            if save_out:
                annotated_image = image.copy()
                for detection in detections:
                    x, y, w, h = detection['box']
                    cv2.rectangle(annotated_image, (x, y), (x + w, y + h), (255, 0, 0), 2)
                cv2.imwrite('mtcnn_facenet_' + img_path.name, annotated_image)
    return time_taken


if __name__ == '__main__':
    time_taken = benchmark_mtcnn_tensorflow(save_out=False)
    print(dict(min=min(time_taken) * 1000, max=max(time_taken) * 1000, mean=statistics.mean(time_taken) * 1000))
